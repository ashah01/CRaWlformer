...
@register_node_encoder('TypeDictNodeOneHot')
class TypeDictNodeOneHotEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
        num_types = cfg.dataset.node_encoder_num_types
        if num_types < 1:
        ...
        self.encoder = torch.nn.Linear(in_features=num_types, out_features=emb_dim)
...
@register_edge_encoder('TypeDictEdgeOneHot')
class TypeDictEdgeOneHotEncoder(torch.nn.Module):
    def __init__(self, emb_dim):
        super().__init__()
        num_types = cfg.dataset.edge_encoder_num_types
        if num_types < 1:
